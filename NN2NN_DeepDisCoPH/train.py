...
from envCT import PHInterconnectionEnv, SystemEnv, Controller,NNController
...
def train_HDNN(n_agents, t_end, steps, min_dist, s, learning_rate, alpha_u, alpha_ca, n_H_layers, n_layers,
    ...
    # # # # # # # # Controller # # # # # # # #
    nnctl=NNController(n_layers=n_layers, h=h, r=r, n_agents=n_agents, interconnection=s, n_H_layers=n_H_layers)
    ctl = Controller(n_layers=n_layers, h=h, r=r, n_agents=n_agents, interconnection=s, n_H_layers=n_H_layers,nncontroller=nnctl)
    ...
    for epoch in range(max_iteration):
        ...
        if not grad_info:
            ...
            print("all-loss: %.1f \t|\t " % (loss + alpha_u*loss_u + alpha_ca*loss_ca))
