...
def plot_trajectories(x, xbar, n_agents, text="", save=False, filename=None):
    ...
    if save:
        plt.savefig('figures/' + filename+'_'+text+'_trajectories.pdf', format='pdf')
    else:
        plt.show()
...
def plot_traj_vs_time(t, n_agents, x, xi=None, text="", save=False, filename=None):
    ...
    plt.legend(legend)
    if save:
        plt.savefig('figures/' + filename+'_'+text+'_x.pdf', format='pdf')
    else:
        plt.show()
    # xi trajectories
    if xi is not None:
        ...
        plt.legend(legend)
        if save:
            plt.savefig('figures/' + filename+'_'+text+'_xi.pdf', format='pdf')
        else:
            plt.show()
...
def plot_energy(t, x, xi, sys, ctl, save=False, filename=None):
    ...
    if save:
        plt.savefig('figures/' + filename + '_energy.pdf', format='pdf')
    else:
        plt.show()
...
def plot_input(t, xi, ctl, save=False, filename=None):
    ...
    plt.ylabel(r'$u_y(t)$')
    if save:
        plt.savefig('figures/' + filename + '_input.pdf', format='pdf')
    else:
        plt.show()
...
def plot_rel_distance(t, x, min_dist, save=False, filename=None):
    ...
    if save:
        plt.savefig('figures/' + filename + '_distances.pdf', format='pdf')
    else:
        plt.show()
...
def plot_grads(end_layer=-1, save=False, filename=None):
    ...
    max_iteration, n, _, stpdf = bsm.shape
    if 1 < end_layer < stpdf:
        stpdf = end_layer
        sub_i = str(int(stpdf))
    else:
        sub_i = 'N'
    bsm_norm = torch.zeros(max_iteration, stpdf)
    for epoch in range(max_iteration):
        for j in range(stpdf - 1):
            bsm_ = bsm[epoch, :, :, j]
            for i in range(j+1, stpdf - 1):
    ...
    #plt.title("BSM")
    if save:
        plt.savefig('figures/' + filename + '_bsm.pdf', format='pdf')
